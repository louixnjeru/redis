...
size_t getHashMapSize(HashMap* h_map) {
    return h_map->newer.size + h_map->older.size;
}
...
void get(std::vector<std::string> &cmd, Conn *conn) {
    ...
    if (!node) {
        buffer_append_error(conn->outgoing, 2, "not found");
        return;
    }
    ...
    buffer_append_string(conn->outgoing, val->data(), val->size());
...
void set(std::vector<std::string> &cmd, Conn *conn) {
    ...
    buffer_append_nil(conn->outgoing);
}
...
void del(std::vector<std::string> &cmd, Conn *conn) {
    ...
    buffer_append_int(conn->outgoing, node? 1 : 0);

}

void keys(std::vector<std::string> &cmd, Conn *conn) {
    buffer_append_array(conn->outgoing, static_cast<u_int32_t>(getHashMapSize(&g_data.db)));
    forEachInHashMap(&g_data.db, combineKeys, &conn->outgoing);
}

void forEachInHashMap(HashMap* h_map, std::function<bool(Node*, Buffer*)> combine, Buffer* buff) {
    forEachInHashTable(&h_map->newer, combine, buff) && forEachInHashTable(&h_map->older, combine, buff);
}

bool forEachInHashTable(HashTable* h_table, std::function<bool(Node*, Buffer*)> combine, Buffer* buff) {
    for (size_t i {0}; h_table->mask != 0 && i <= h_table->mask; i++) {
        for (Node* node {h_table->table[i]}; node != NULL; node = node->next) {
            if (!combine(node, buff)) {
                return false;
            }
        }
    }
...
bool combineKeys(Node* node, Buffer* buff) {
    ...
    buffer_append_string(*buff, key.data(), key.size());
    return true;
}